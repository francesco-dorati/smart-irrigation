...
{
public:
  RS485Client(String id, int comm, int rx, int tx) : id(id), comm(comm), serial(rx, tx) {}
  ...
  { // Ensure command is from this plant
    ...
    return (recipient.equals(id) || recipient.equals("*"))
               ? getBody(command)
               : "";
  }
...
void loop()
{
  ...
  if (rs485.available())
  {
    String command = rs485.receiveCommand(); // Ensure command is for this plant
    if (!command.equals(""))
    {
...
void interpretCommand(String command)
{
  ...
  else if (command.equals("HUMIDITY"))
  {
    ...
    rs485.transmit(String(humidity) + " " + String(humiditySensor.getVoltage()));
  }
...
void interpretDebug(String command)
{
  ...
  else if (command.equals("HUMIDITY"))
  {
    ...
    int humidity = humiditySensor.getHumidity();
    Serial.println("Humidity: " + String(humidity) + "%, Voltage: " + String(voltage) + "V, value: " + String(value));
  }
  else if (command.startsWith("WATER"))
  {
    int seconds = command.substring(6).toInt();
